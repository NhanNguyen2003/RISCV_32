...
public class RV32iCpuCSRTest {
    ...
    public void testCSRImmediate() throws Exception {
        ...
        // CSRRWI x1, mstatus, 10 (Read mstatus into x1, write immediate value 10 to
        // mstatus)
        System.out.println("Step 1: CSRRWI x1, mstatus, 10");
        System.out.println("  Before: MSTATUS = 0x" + Integer.toHexString(cpu.readCSRTest(RV32Cpu.MSTATUS)));
        int csrrwiInstruction = 0b00110000000001010101000011110011;
        cpu.testExecuteInstruction(csrrwiInstruction);
        System.out.println("  After: MSTATUS = 0x" + Integer.toHexString(cpu.readCSRTest(RV32Cpu.MSTATUS)));
        System.out.println("  x1 = " + cpu.getRegister(1) + " (should be 0x1800 = " + 0x1800 + ")");

        // Test CSRRSI instruction (CSR Read and Set Immediate)
        // CSRRSI x2, mstatus, 8 (Read mstatus into x2, set bits from immediate value 8
        // in mstatus)
        ...
        // CSRRCI x3, mstatus, 10 (Read mstatus into x3, clear bits from immediate value
        // 10 in mstatus)
    ...
    public void testPrivilegeModeSwitch() throws Exception {
        ...
        cpu.writeCSRTest(RV32Cpu.MSTATUS, 0x0);
        // Set up mepc with a return address
        cpu.writeCSRTest(RV32Cpu.MEPC, 0x1000);
        ...
        assertEquals(RV32Cpu.PRIVILEGE_USER, cpu.getPrivilegeMode(), "CPU should be in U-mode after MRET");
    ...
    public void testExceptionHandling() throws Exception {
        ...
        cpu.writeCSRTest(RV32Cpu.MTVEC, 0x2000);
        ...
        assertEquals(RV32Cpu.PRIVILEGE_MACHINE, cpu.getPrivilegeMode(), "CPU should be in M-mode after exception");
        ...
        assertEquals(0x1000, cpu.readCSRTest(RV32Cpu.MEPC),
                "mepc should contain the address of the faulting instruction");

        // Check that mcause contains the exception cause
        assertEquals(2, cpu.readCSRTest(RV32Cpu.MCAUSE), "mcause should contain the exception cause");
    }
}